...
CPopupMenu::~CPopupMenu()
{
	if (m_iSpawnItem > 0)
	{
		for (int Item=0; Item<m_iSpawnItem; Item++)
		{
			if (m_pSpawnItem[Item]) 
				delete m_pSpawnItem[Item];
		}
		
		GlobalFree(static_cast<HGLOBAL>(m_pSpawnItem));
	}

	if (m_iImageItem > 0)
	{
		GlobalFree(static_cast<HGLOBAL>(m_pImageItem));
	}

	if (m_hMenuFont) 
		::DeleteObject(static_cast<HGDIOBJ>(m_hMenuFont));
...
void CPopupMenu::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	if (lpDrawItemStruct->CtlType == ODT_MENU)
	{
		...
		if (pItem)
		{
			...
			// David 08/04/98 - start - bold font handling
			if (!bBold)
				pft = CFont::FromHandle(static_cast<HFONT>(m_hMenuFont) ? m_hMenuFont : m_hGuiFont);
			else 
				pft = CFont::FromHandle(static_cast<HFONT>(hMenuBoldFont) ? hMenuBoldFont : m_hGuiFont);
			...
			else
			{
				...
				{
					int ay = (rcImage.Height() - m_szImage.cy) / 2;
					int ax = (rcImage.Width()  - m_szImage.cx) / 2;
					...
					if (bEnab)
					{
						m_ilList.Draw(pDC, pItem->iImageIdx, CPoint(rcImage.left + ax, 
							rcImage.top +ay), ILD_NORMAL);
					}
					else
					{
						HICON hIcon = m_ilList.ExtractIcon(pItem->iImageIdx);
						pDC->DrawState(CPoint(rcImage.left + ax, rcImage.top + ay), m_szImage, 
		...
		if (pItem)
		{
			...
			else
			{
				...
				if (!cs.IsEmpty())
				{
					...
					{
						lpMeasureItemStruct->itemHeight = m_szImage.cy + 5;
						...
						lpMeasureItemStruct->itemWidth += lpMeasureItemStruct->itemHeight > static_cast<UINT>(m_szImage.cx)
							? static_cast<UINT>(lpMeasureItemStruct->itemHeight) : static_cast<UINT>(m_szImage.cx);
					}
					dc.SelectObject(of);
				}
				else
...
BOOL CPopupMenu::CreateBoldFont()
{
	...
	LOGFONT lgFont;
	::GetObject (m_hMenuFont ? m_hMenuFont : m_hGuiFont, sizeof(lgFont), &lgFont);
...
BOOL CPopupMenu::AddToolBarResource(UINT resId)
{
	...
	for (int ItemNum = 0; ItemNum < pTBData->wItemCount; ItemNum++, pItem++)
	{
...
BOOL CPopupMenu::LoadToolBarResource(UINT resId)
{
	...
	m_szImage.cx = static_cast<int>(pTBData->wWidth);
	m_szImage.cy = static_cast<int>(pTBData->wHeight);
	// 创建位图列表
	if (m_ilList.Create(m_szImage.cx, m_szImage.cy, ILC_COLOR4 | ILC_MASK, pTBData->wItemCount, 0) == FALSE)
		return FALSE;
	// 设置位图的背景色
	m_ilList.SetBkColor(cr3dFace);
	...
	bmp.LoadBitmap(resId);
	// 往位图列表中加入位图
	m_ilList.Add(&bmp, RGB(0, 192, 192));
	bmp.DeleteObject();
...
void CPopupMenu::AddImageItem(const int idx, WORD cmd)
{
	...
	if (m_iImageItem == 0)
		m_pImageItem = static_cast<ImageItem*>(GlobalAlloc(GPTR, sizeof(ImageItem)));
	else
		m_pImageItem = static_cast<ImageItem*>(GlobalReAlloc(static_cast<HGLOBAL>(m_pImageItem), 
		sizeof(ImageItem) * (m_iImageItem + 1), GMEM_MOVEABLE | GMEM_ZEROINIT));
	
	ASSERT(m_pImageItem);
	m_pImageItem[m_iImageItem].iCmd      = static_cast<int>(cmd);
	m_pImageItem[m_iImageItem].iImageIdx = idx;
	m_iImageItem++;
}
...
{
	if (m_iSpawnItem == 0)
		m_pSpawnItem = static_cast<SpawnItem**>(GlobalAlloc(GPTR, sizeof(SpawnItem)));
	else
		m_pSpawnItem = static_cast<SpawnItem**>(GlobalReAlloc(static_cast<HGLOBAL>(m_pSpawnItem), 
		sizeof(SpawnItem) * (m_iSpawnItem + 1), GMEM_MOVEABLE | GMEM_ZEROINIT));
	
	ASSERT(m_pSpawnItem);
	
	SpawnItem* p = new SpawnItem;
	ASSERT(p);
	m_pSpawnItem[m_iSpawnItem] = p;
	lstrcpy(p->cText, txt);
	p->iCmd = cmd;
...
{
	for (int ItemNum=0; ItemNum<m_iImageItem; ItemNum++)
	{
		if (m_pImageItem[ItemNum].iCmd == cmd)
			return m_pImageItem[ItemNum].iImageIdx;
	}
	return -1;
}
