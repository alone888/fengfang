...
BOOL CCfgCtrlBar::Create(CWnd* pParentWnd, CSize sizeDefault, UINT nID, DWORD dwStyle) 
{
    ...
    if (!CWnd::Create(wndclass, _T("配置与分析条"), dwStyle, CRect(0, 0, 0, 0), 
        pParentWnd, nID))
        return FALSE;
...
// bStretch = 0表示该控制条可用于浮动或停靠,则不能拉伸
...
{
	// 只要在停靠状态下一旦父窗体改变大小时，就发生此事件，父窗体移动则不发生此事件
	// 在此事件下，其Length总是为-1
    if (dwMode & (LM_HORZDOCK | LM_VERTDOCK)) 
	{
		...
		// 返回最大宽度的动态尺寸或最大高度的动态尺寸
        return CControlBar::CalcDynamicLayout(nLength, dwMode);
	}

    if (dwMode & LM_MRUWIDTH)
	{
		// 使用最近使用宽度或高度(在停靠状态和浮动状态之间变换时发生)
        return m_sizeFloat;
	}
	...
	// 当在浮动状态下改变垂直高度Y时发生此事件,切在拖动其上边框或下边框的过程中反复产生此事
	// 件,其Length为鼠标当前位置高度
    if (dwMode & LM_LENGTHY)
	{
		m_sizeFloat.cy = max(m_sizeMin.cy, nLength);
        return CSize(m_sizeFloat.cx, m_sizeFloat.cy);
	}
    else // 在浮动状态下，若改变的是其宽度X，则不断发生此事件，至到左边框或右边框拖动结束
	{
...
LRESULT CCfgCtrlBar::OnNcHitTest(CPoint point) 
{
