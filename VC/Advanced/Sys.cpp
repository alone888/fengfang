...
ON_COMMAND(ID_TOOLSBAR_NEW_FILE, OnDataFileNew)
ON_COMMAND(ID_TOOLSBAR_OPEN_FILE, OnOpenADHist)
ON_COMMAND(ID_TOOLSBAR_START,CSysApp::OnStartDeviceAD)
ON_COMMAND(ID_TOOLSBAR_STOP, CSysApp::OnStopDeviceAD)
...
void CSysApp::OnAppAbout()
{
	...
	//aboutDlg.DoModal();
		...
		pWaveView->StartScopeCtrlTimer();//	启动timer 测试画图用
}
